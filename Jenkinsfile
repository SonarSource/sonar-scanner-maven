  ...
  stages {
    ...
    stage('Build PR'){
      ...
      steps {
        withSonarQubeEnv('next') {
          withMaven(maven: MAVEN_TOOL) {
            sh "mvn clean org.jacoco:jacoco-maven-plugin:prepare-agent package sonar:sonar -Dsonar.branch.name=${env.BRANCH_NAME}       -Dsonar.pullrequest.branch=${env.BRANCH_NAME} \
